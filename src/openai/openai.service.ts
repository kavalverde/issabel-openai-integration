...
export class OpenaiService {
  ...
  async textToSpeech(
    ...
    try {
      this.logger.log(
        `Convirtiendo texto a voz: "${request.text.substring(0, 30)}..."`,
      );
  ...
  async processConversation(audioFilePath: string): Promise<string> {
    try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'Eres un asistente telefónico amable y eficiente que ayuda a los clientes con sus consultas. Proporciona respuestas claras y concisas.',
          },
          {
            role: 'user',
      ...
      this.logger.error(
        `Error en el procesamiento de la conversación: ${error.message}`,
      );
      throw error;
    }
  }
        ...
        messages: [
          {
            role: 'system',
            content:
              'Eres un asistente telefónico amable y eficiente de la empresa Node Analytics. Que tiene un conjunto de herramientas para gestión de SAC y la insignia del servicio es la omnicanalidad, enfoncandose en canales escritos de redes sociales que incluyen Facebook, instragam y Whatsapp, además de los canales tradicionales de voz en call center y correo electronico, el beneficio más grande es la flexibilidad para poder adaptar la plataforma a los clientes, ya que la empresa es propietaria del codigo de la plataforma. Si te piden que quieres hablar con un agente humano response: "Claro, te puedo ayudar con eso, por favor espera un momento mientras te transfiero con un agente.". Si notas que el cliente está interesado en el producto, response: "Claro, puedo ayudarte con eso, un agente se contactará contigo para brindarte más información. ¿Puedo ayudarte con algo adicional?". Si el cliente tiene intención de despedirse, response: "Gracias por contactarnos, que tengas un buen día."',
          },
          {
            role: 'user',
            content: 'Te agradezco por tu tiempo, pero no tengo más preguntas.',
          },
        ],
      });
